...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare complete shipping address string for the order
      ...
      // Create full shipping details object
      ...
      // Payment details (for local storage only - don't send full card details to server)
      ...
      // Calculate totals for the order confirmation page
      const subtotal = calculateSubtotal();
      const shipping = calculateShippingCost(formData.shippingMethod);
      const tax = calculateTax();
      const total = subtotal + shipping + tax;

      // Store complete order data in localStorage for fallback
      const completeOrder = {
        ...response,
        items: cartItems.map((item) => ({
          ...item,
      ...
      // Check if response has an ID property, otherwise use a fallback
      const orderId = response?.id || response?.order_id || "new";
